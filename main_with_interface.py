...
def chooseRecordingDevice():
    ...
    for i in range(0, data.get('deviceCount')):
        if (pyAudio.get_device_info_by_host_api_device_index(0, i).get('maxInputChannels')) > 0:
            list_of_devices.append(pyAudio.get_device_info_by_host_api_device_index(0, i).get('name'))
    #device = int(input())
...
def extract_features(audio,rate):
    ...
    mfcc_feature = mfcc.mfcc(audio,rate, 0.025, 0.01,39,nfft = 1200, appendEnergy = True)    
...
def add_user():
    ...
    name_label = Label(wsnew, text = 'Nazwa użytkownika', font=('calibre',10, 'bold'))
    name_entry = Entry(wsnew, textvariable = modelName, font=('calibre',10,'normal'))
    name_label.place(relx=0.4, rely=0.3, anchor=CENTER)
    name_entry.place(relx=0.6, rely=0.3, anchor=CENTER)

    label_info=Label(wsnew, text="Wciśnij przycisk i wypowiedź wyświetlone poniżej hasło:", font='Arial 15')
    ...
    try: 
        start_btn = Button(
            ...
            text='START', 
            bd='5',
            command = lambda: create_profile(wsnew, name_entry.get(), label_password, label_info)
            )
...
def navigation(window_name):
    ...
    mainmenu.add_command(label = "Identifikuj nagrane próbki", command= train_test)
...
label_info=Label(ws, text="Wciśnij przycisk i wypowiedź wyświetlone poniżej hasło:", font='Arial 15')
label_info.place(relx=0.5, rely=0.4, anchor=CENTER)
